...
public class KotlinLanguageProvider : IModLanguageProvider {
    ...
    private class KotlinModTarget(private val className: String) : IModLanguageProvider.IModLanguageLoader {
        override fun <T> loadMod(info: IModInfo, modFileScanResults: ModFileScanData, gameLayer: ModuleLayer): T {
            try {
                val ktContainer = Class.forName("thedarkcolour.kotlinforforge.neoforge.KotlinModContainer", true, Thread.currentThread().contextClassLoader)
        ...
        private fun catastrophe(info: IModInfo, exception: Exception): Nothing {
            ...
            // ModLoadingException
            val mle = Class.forName("net.neoforged.fml.ModLoadingException", true, Thread.currentThread().contextClassLoader) as Class<ModLoadingException>
            // ModLoadingStage
            val mls = Class.forName("net.neoforged.fml.ModLoadingStage", true, Thread.currentThread().contextClassLoader) as Class<ModLoadingStage>
